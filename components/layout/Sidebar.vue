<template>
  ...
  <div class="sidebar__head">
    <svg class="sidebar__head-logo" xmlns="http://www.w3.org/2000/svg"
    ...
    <h3 class="t-h3 sidebar__head-title">{{ 'Vollary' }}</h3>
  </div>
  <div class="sidebar__body">
    <div v-for="(el, idx) in SIDEBAR_TREE" :key="idx">
      <h3 class="sidebar__header">{{ el.title }}</h3>
      <div class="sidebar__item-container">
        <sidebar-link v-for="(link, linkIdx) in el.links" :key="linkIdx" :link="link.link" :icon="link.icon">
          {{ link.label }}
        </sidebar-link>
      </div>
    </div>
